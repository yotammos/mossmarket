...
public class ListItems implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(final APIGatewayProxyRequestEvent requestEvent,
        ...
        if (requestEvent == null || context == null) {
            return responseEvent.withBody("null argument found").withStatusCode(HttpURLConnection.HTTP_BAD_REQUEST);
        }
        ...
        try {
            ...
            return responseEvent.withBody(gson.toJson(items)).withStatusCode(HttpURLConnection.HTTP_OK);
        } catch (final JsonSyntaxException ignored) {
            return responseEvent.withBody("invalid request format").withStatusCode(HttpURLConnection.HTTP_BAD_REQUEST);
        } catch (final Exception e) {
            return responseEvent.withBody(e.getMessage()).withStatusCode(HttpURLConnection.HTTP_INTERNAL_ERROR);
        }
    }
}
